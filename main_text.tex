...
Such an approach allows us to analyze prior data from high-throughput screens, and show that the unperturbed clock has been highly optimized to be both accurate and high amplitude.
...
Within each tissue, cellular-level rhythms in gene transcription are generated by a large network of interacting regulatory elements, in which time-delayed negative feedback gives rise to sustained oscillations \cite{Ueda2005}.
...
Additionally, as the amplitude of circadian transcription can be affected by lifestyle variables such as diet, age, or work schedules, there has been recent interest in developing pharmacological strategies for increasing the amplitude of circadian cycles in metabolic tissues \cite{Chen2013}.
...
For circadian systems, intrinsic noise has been shown to play a larger role: a single cell's variability in period from cycle-to-cycle has been shown to be larger than the variability in mean period length between cells \cite{Herzog2004}.
In cell cultures which lack cell-to-cell coupling, it has been shown that  stochastic noise at the single-cell level is manifested in damped oscillations at the population-level as individual oscillators gradually drift out of phase \cite{Nagoshi2004, Welsh2004}. 
...
We first sought to determine whether changes to intrinsic stochastic noise are sufficient to explain population-level changes in damping rate.
...
\begin{figure}[tbp]
  \begin{center}
    \makebox[\textwidth][c]{\includegraphics[]{figures/pdfs/mainfig_dose.pdf}}
  \end{center}
...
\subsection*{Mathematical model predicts KL001 damping rate increase comes from increased single-cell stochastic noise}
...
This computational result 

\begin{figure}[tbp]
  \begin{center}
    \makebox[\textwidth][c]{\includegraphics[]{figures/pdfs/main_fig_simulations.pdf}}
  \end{center}
...
While it is true that the dephasing of a group of oscillators can be caused by both variance in the mean period as well as cycle-to-cycle variability, experimental evidence suggests intrinsic stochastic noise plays a more significant role.
We show that there is greater variance in period on a cycle-to-cycle basis than on a cell-to-cell basis in cultured fibroblast cells (Figure~\ref{fig:cell2cell-variability}).
...
Such a change, caused perhaps by an unequal uptake of drug across cells, would result in an altered distribution in mean period length, and different dephasing kinetics.

% Additionally, it is important to note that the dephasing kinetics of these two alternative processes result in different dynamics at the population-level.
...
This type of anomalous diffusion results in a different damping profile, as highlighted in Figure~\ref{fig:dephasing-rates}.
Since the data used in this study is well-described by an exponentially damped sinusoid, and since a mathematical model assuming homogeneous mean periods accurately predicts experimental results, we expect our assumption that changes to intrinsic noise are primarily responsible for the observed damping rate changes to be justified. 

However, it is possible that damping rate changes due to siRNA or small molecule perturbation are manifested by altering the system's extrinsic noise.
Such a change, caused perhaps by an unequal uptake of drug across cells, would result in an altered distribution in mean period length, and different dephasing kinetics.
A simple model of a stochastic oscillator was used to investigate the effects of altering intrinsic and extrinsic noise on the dephasing kinetics (Figure~\ref{fig:computational_dephasing}).
While {\itshape in silico} dephasing due to solely intrinsic or extrinsic sources can be differentiated from population-level rhythms, it becomes more difficult to identify noise sources when multiple types of noise are present.
The window function of population-level oscillations can be affected by many factors, including weak cell-to-cell coupling \cite{Rougemont2007}, temporary amplitude change \cite{St.John2014a}, and intrinsic and extrinsic noise.
While altering intrinsic, or cell-autonomous, noise has been shown to be sufficient to explain the observed damping rate changes, it is possible that a portion of the changes to damping rate observed in regressed experimental data may be due to changes in extrinsic noise.
...
\begin{figure}[tbp]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figures/pdfs/FigS7.pdf}
  \end{center}
  \caption{{\bfseries Intrinsic and extrinsic noise can be difficult to differentiate in .} ({\bfseries A}) }
